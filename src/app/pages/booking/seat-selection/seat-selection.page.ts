...
import { arrowBackOutline, chevronForwardOutline, navigateOutline } from 'ionicons/icons';
...
interface Seat { id: string; row: number; status: SeatStatus; }
...
const SHUFFLE_ORDER = [
  '3B','7A','2D','9C','1A','5C','8B','4D','6A','10B','2A','7D','3C','9A','5B','1D',
  '8C','4A','6D','10C','2C','7B','3A','9D','5A','1C','8D','4B','6C','10A','2B','7C',
  '3D','9B','5D','1B','8A','4C','6B','10D'
];
...
export class SeatSelectionPage implements OnInit {
  ...
  buildSeatMap() {
    const seatsLeftNum = Number((this.booking.trip!.seatsLeft || '').replace(/[^0-9]/g, '')) || 20;
    ...
    for (let r = 1; r <= ROWS; r++) {
      for (const c of COLS) {
        ...
        if (status === 'available' && alreadySelected.has(id)) status = 'selected';
  ...
  leftPair(row: number): Seat[] { return this.seatsInRow(row).slice(0, 2); }
  rightPair(row: number): Seat[] { return this.seatsInRow(row).slice(2, 4); }
  ...
  toggleSeat(seat: Seat) {
    ...
    if (seat.status === 'selected') {
      ...
      this.booking.selectedSeats = this.booking.selectedSeats.filter(id => id !== seat.id);
    } else {
      if (this.booking.selectedSeats.length >= this.booking.passengerCount) return;
  ...
  goBack() { this.router.navigateByUrl('/booking/trip'); }
